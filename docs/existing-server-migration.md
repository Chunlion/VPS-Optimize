...
这篇文档面向已经跑了服务的 VPS：已有 3x-ui、Caddy/Nginx 网站、订阅工具、Docker Compose 项目，想把公网 `443` 统一交给 VPS-Optimize 的 443端口复用。
...
| 域名清单 | 面板域名、面板域名下的 3x-ui 订阅 URI、网站域名、节点域名；如使用独立订阅工具，再记录其单独域名 |
| 后端清单 | 每个服务的本地监听地址和端口 |
...
## 未启用 443端口复用时的 HTTPS 反代过渡

如果你暂时不准备启用 443端口复用，只是想先给订阅工具或网站加 HTTPS 域名，可以走独立反代入口：
...
| `[2 添加 Nginx HTTPS 反代]` | 未启用 443端口复用，且希望由 Nginx 直接监听公网 80/443 |
...
1. 该流程只适合还没有启用 443端口复用的服务器。
2. 如果已经启用 443端口复用，Nginx HTTPS 反代会抢占公网 `443`，应改用 `主菜单 [19 443端口复用管理中心] -> [6 管理 Web 域名/反代]`，并在该菜单里选择 Caddy 或 Nginx 本地 Web 反代引擎。
...
首次配置或重新应用 443端口复用可能会隔离旧 Caddy 配置和脚本管理的旧 Nginx HTTPS 反代配置，避免旧配置继续抢占公网 `443`。启用后不要再手写抢占 `443` 的旧规则，而是逐个补录：

```text
主菜单 [19 443端口复用管理中心] -> [6 管理 Web 域名/反代]
```
...
443端口复用模式下，公网 `443` 应由当前入口模式统一监听。旧 Nginx server、Apache、面板、Xray 都不应该再直接监听公网 `443`；如果希望继续用 Nginx 做网站反代，请在 `[19] -> [6]` 切换到 Nginx 本地 Web 反代引擎，而不是保留旧的公网 `443` server。

建议做法：

1. 让旧网站服务改为本机 HTTP 后端，例如 `127.0.0.1:8080`。
2. 用 443端口复用的“管理 Web 域名/反代”把域名反代到该后端。
3. 验证公网 HTTPS 域名。
4. 确认无误后再收紧防火墙和旧公网端口。
...
目标是容器后端只在本机或内网监听，公网只走 Caddy/Nginx 反代或 443端口复用。启用 443端口复用后，Caddy/Nginx 是可选的本地 Web 反代引擎，不再直接抢公网 `443`。
...
```text
主菜单 [11 Docker 管理] -> [10 Docker 本地防穿透]
```
...
选择 `[1 添加 Caddy 反代]` 或 `[2 添加 Nginx HTTPS 反代]`，把独立订阅工具域名反代到本地后端端口。Nginx HTTPS 反代会申请或复用 `/etc/caddy/certs/${domain}.crt|key` 证书，并由 Nginx 直接监听公网 80/443；该方式不能和 443端口复用同时抢公网 `443`。

如果已经启用或准备启用 443端口复用，则通过 443端口复用新增外部域名：
...
| 443 体检 | `主菜单 [19 443端口复用管理中心] -> [11 443 链路体检]` | 无关键失败 |
...
| 新增网站时重跑首次配置 | 配置被重复改写，排错变复杂 | 后续新增只走 `[6 管理 Web 域名/反代]` |
...
| 没有迁移旧 Caddy/Nginx 站点 | 旧网站启用 443 后打不开 | 逐个通过 `[6 管理 Web 域名/反代]` 补录，并选择需要的 Web 反代引擎 |
