...
首次使用 `[2 安装 / 切换 443 入口模式]` 时可以选择 Caddy 或 Nginx 作为 443 单入口下的本地 Web 反代引擎。后续也可以从：
...
```text
主菜单 [19 443 单入口管理中心] -> [2 安装 / 切换 443 入口模式] -> [3 TCP Peek + Splice]
```

脚本会自动安装缺少的构建依赖、执行配置校验和 `8444` 预检，通过后再切换公网 `443`。如需撤销最近一次入口模式切换，使用 `主菜单 [19 443 单入口管理中心] -> [7 回滚上一次入口模式切换]`。
...
4. 配置 REALITY 入站
5. 进入 `主菜单 [19 443 单入口管理中心] -> [2 安装 / 切换 443 入口模式]`
