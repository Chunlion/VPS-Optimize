...
首次部署时，把 Token 粘贴到脚本的 `CF Token` 提示处，不要填写邮箱、Zone ID 或 Global API Key。已经部署过时，可进入主菜单 `[19 443端口复用管理中心]` → `[10 CF DNS / Caddy 证书维护]` → `[8 更新 Cloudflare API Token]`。脚本会在线校验 Token；校验失败时优先检查权限、授权 Zone 和可选的 IP 限制。
...
```text
主菜单 [19 443端口复用管理中心] -> [4 回滚上一次入口模式切换]
```
...
```text
主菜单 [19 443端口复用管理中心] -> [6 管理 Web 域名/反代] -> [8 切换 Web 反代引擎]
```
...
```text
主菜单 [19 443端口复用管理中心] -> [6 管理 Web 域名/反代] -> [5 管理域名 IP 白名单]
```
...
```text
主菜单 [19 443端口复用管理中心] -> [15 SNI 清洗 / REALITY 防护]
```
...
3. 打开 `主菜单 [19 443端口复用管理中心] -> [13 Xray 入站管理]`，分别添加两条 `SNI -> 本地地址 -> 本地端口` 记录。每次保存后，脚本都会提示是否立即同步到当前入口；连续添加多条时可以先不应用，全部录入后再执行一次“同步到当前入口模式”。
4. 打开 `主菜单 [19 443端口复用管理中心] -> [15 SNI 清洗 / REALITY 防护] -> [4 设置 REALITY 回落限速]`。菜单会列出所有 REALITY 入站；每次选择一个入站，设置只会应用到当前选中的入站，两个入站都要保护就重复执行两次。
...
6. 在脚本中使用 `主菜单 [19 443端口复用管理中心] -> [11 443 链路体检]` 查看入口、证书、Web 和 Xray 路由；需要测试公网 DNS/TCP/TLS 时使用 `[12 443 网络访问测试]`。
