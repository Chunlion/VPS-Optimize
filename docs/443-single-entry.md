...
| 已启用 443 Nginx stream 单入口 | `主菜单 [18] -> [8] 管理域名 IP 白名单`，或 `[18] -> [2] -> [5]` | Nginx stream 层，按 `SNI + 源 IP` 判断 | 只影响被选择的 SNI 域名 |
...
4. 配置 REALITY 入站
5. 进入 `主菜单 [18 443 单入口管理中心] -> [1 首次配置 443 单入口]`
6. 回到 3x-ui 收尾：监听改本机、订阅反代 URI、External Proxy
7. 进入 `主菜单 [18 443 单入口管理中心] -> [3 443 单入口链路体检]`
```
...
| 检查 443 链路 | `主菜单 [18 443 单入口管理中心] -> [3 443 单入口链路体检]` |
| 修改面板/订阅端口与路径 | `主菜单 [18 443 单入口管理中心] -> [7 修改 443 分流参数] -> [1 修改面板/订阅端口与路径]` |
| 修改 REALITY 本地监听 / 伪装 SNI | `主菜单 [18 443 单入口管理中心] -> [7 修改 443 分流参数] -> [2 修改 REALITY 本地监听 / 伪装 SNI]` |
| 修改 Nginx / Caddy 监听 | `主菜单 [18 443 单入口管理中心] -> [7 修改 443 分流参数] -> [3 修改 Nginx 公网入口 / Caddy 本地 TLS]` |
| 修改面板域名 | `主菜单 [18 443 单入口管理中心] -> [7 修改 443 分流参数] -> [4 修改面板域名]` |
| 重新应用当前配置 | `主菜单 [18 443 单入口管理中心] -> [7 修改 443 分流参数] -> [5 重新应用当前保存的配置]` |
| 证书维护 | `主菜单 [18 443 单入口管理中心] -> [6 CF DNS / Caddy 证书维护]` |
| 回滚 443 单入口配置 | `主菜单 [18 443 单入口管理中心] -> [6 CF DNS / Caddy 证书维护]` 中的回滚入口 |
