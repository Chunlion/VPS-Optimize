...
2. 第一次使用 TCP Peek 前，先运行 `主菜单 [19] -> [16] TCP Peek 8444 预检 / 安装测试`。这个步骤只监听 `8444`，不会替换公网 `443`。
3. `[16]` 通过后，再运行 `[5] 切换到 TCP Peek + Splice 模式`。切换前脚本还会自动再跑一次独立 `8444` 预检，失败就不动公网 `443`。
...
新机器首次配置建议先选 Nginx Stream。等共享配置、证书、Caddy 和 3x-ui 本地端口都跑通后，再按上面的 `[16] -> [5]` 流程切到 TCP Peek。
...
TCP Peek + Splice 额外要求：

```text
主菜单 [19 443 单入口管理中心] -> [16 TCP Peek 8444 预检 / 安装测试]
主菜单 [19 443 单入口管理中心] -> [17 TCP Peek 分流规则校验]
```

只有 `8444` 预检和分流规则校验通过后，再执行 `[5] 切换到 TCP Peek + Splice 模式`。
