...
这份手册用于处理已经出问题的情况：SSH 断开、防火墙误封、443端口复用改坏、Nginx 入口或 Caddy/Nginx Web 反代引擎起不来、证书失败、面板打不开。它的目标是先恢复可控状态，再排查根因。
...
```text
主菜单 [15 服务健康总览] -> [s 服务恢复] -> [r 重启一个常用服务]
主菜单 [15 服务健康总览] -> [s 服务恢复] -> [f 重启失败服务]
主菜单 [15 服务健康总览] -> [s 服务恢复] -> [l 查看服务日志]
```

如果是 443端口复用问题，优先：

```text
主菜单 [19 443端口复用管理中心] -> [11 443 链路体检]
```
...
`主菜单 [8 防火墙规则管理] -> [5 端口并发连接限制]` 优先使用原生 nftables `ct count`，不支持时回退到 `iptables` / `ip6tables` connlimit。该限制不等同于 UFW/firewalld 端口放行。它按公网端口限制每来源 IP 的 TCP 并发连接数；如果限制公网 `443` 且启用了 443端口复用，只能限制整个公网 `443`，不能精准到某个 Xray/3x-ui 入站、SNI、UUID 或用户。不要误选 `[6 关闭防火墙]`。
...
| 目标 | 菜单路径 |
|---|---|
| 新增或删除网站 | `主菜单 [19 443端口复用管理中心] -> [6 管理 Web 域名/反代]` |
| 切换 Caddy/Nginx Web 反代引擎 | `主菜单 [19 443端口复用管理中心] -> [6 管理 Web 域名/反代] -> [8 切换 Web 反代引擎]` |
| 重新生成配置 | `主菜单 [19 443端口复用管理中心] -> [3 重新应用当前入口模式]` |
| 修改面板/订阅/REALITY 参数 | `主菜单 [19 443端口复用管理中心] -> [8 修改 443端口复用参数]` |
| Caddy/证书维护 | `主菜单 [19 443端口复用管理中心] -> [10 CF DNS / Caddy 证书维护]` |
| 回滚 443 配置 | `主菜单 [19 443端口复用管理中心] -> [10 CF DNS / Caddy 证书维护] -> [6 回滚 443端口复用配置]` |

### 切换 Caddy/Nginx Web 反代引擎后坏了

脚本管理的 443 配置以 `/etc/vps-optimize/sni-stack.env` 为准。通过 `[19] -> [6] -> [8]` 切换 Web 反代引擎时，会读取已经保存的面板、订阅、网站、证书、白名单和后端参数重建目标引擎配置；不会反向解析手写的 Caddy/Nginx 配置。

如果之前手动改过 `/etc/caddy/conf.d`、`/etc/nginx/conf.d/vps_sni_web_*.conf` 或旧 `[4 反代]` 生成的 `/etc/nginx/conf.d/vps_proxy_*.conf`，先通过 `[6 管理 Web 域名/反代]` 或 `[8 修改 443端口复用参数]` 把参数同步到脚本，再切换。切换到 Nginx 本地 Web 反代会隔离脚本管理的 Caddy 443 Web 配置；切换回 Caddy 会隔离脚本管理的 Nginx 本地 Web 配置。重新应用 443 时也会隔离旧 Nginx HTTPS 公网反代配置，避免继续抢公网 `443`。
...
```text
主菜单 [19 443端口复用管理中心] -> [10 CF DNS / Caddy 证书维护] -> [15 隔离旧 Caddy 配置]
主菜单 [19 443端口复用管理中心] -> [3 重新应用当前入口模式]
```
...
| 问题 | 菜单路径 |
|---|---|
| 只是 Caddy 语法或重载问题 | `主菜单 [19 443端口复用管理中心] -> [10 CF DNS / Caddy 证书维护] -> [12 校验并重载 Caddy]` |
| Nginx 本地 Web 反代语法或重载问题 | `主菜单 [19 443端口复用管理中心] -> [3 重新应用当前入口模式]` |
| 需要切换 Caddy/Nginx Web 反代引擎 | `主菜单 [19 443端口复用管理中心] -> [6 管理 Web 域名/反代] -> [8 切换 Web 反代引擎]` |
| 后端端口或路径写错 | `主菜单 [19 443端口复用管理中心] -> [8 修改 443端口复用参数] -> [1 修改面板/订阅端口与路径]` |
| 证书文件或软链接异常 | `主菜单 [19 443端口复用管理中心] -> [10 CF DNS / Caddy 证书维护] -> [10 重建 /root/cert 证书软链接]` |
| 不知道是哪类问题 | `主菜单 [19 443端口复用管理中心] -> [11 443 链路体检]` |
...
```text
主菜单 [19 443端口复用管理中心] -> [10 CF DNS / Caddy 证书维护] -> [8 更新 Cloudflare API Token]
主菜单 [19 443端口复用管理中心] -> [10 CF DNS / Caddy 证书维护] -> [9 重新签发某个域名证书]
主菜单 [19 443端口复用管理中心] -> [10 CF DNS / Caddy 证书维护] -> [13 Caddy/证书一键体检]
```
...
主菜单 [19 443端口复用管理中心] -> [8 修改 443端口复用参数] -> [1 修改面板/订阅端口与路径]
主菜单 [19 443端口复用管理中心] -> [11 443 链路体检]
```
...
```text
主菜单 [19 443端口复用管理中心] -> [9 订阅链接 / External Proxy 提示]
主菜单 [19 443端口复用管理中心] -> [8 修改 443端口复用参数] -> [1 修改面板/订阅端口与路径]
```
...
```text
主菜单 [19 443端口复用管理中心] -> [8 修改 443端口复用参数] -> [2 修改 REALITY 本地监听 / 伪装 SNI]
主菜单 [19 443端口复用管理中心] -> [3 重新应用当前入口模式]
主菜单 [19 443端口复用管理中心] -> [11 443 链路体检]
```
...
```text
主菜单 [19 443端口复用管理中心] -> [10 CF DNS / Caddy 证书维护] -> [6 回滚 443端口复用配置]
```
