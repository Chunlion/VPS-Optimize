...
| 目标 | 菜单路径 |
|---|---|
| 新增或删除网站 | `主菜单 [19 443端口复用管理中心] -> [8 管理 Web 域名/反代]` |
| 切换 Caddy/Nginx Web 反代引擎 | `主菜单 [19 443端口复用管理中心] -> [8 管理 Web 域名/反代] -> [8 切换 Web 反代引擎]` |
| 重新生成配置 | `主菜单 [19 443端口复用管理中心] -> [6 重新应用当前入口模式]` |
| 修改面板/订阅/REALITY 参数 | `主菜单 [19 443端口复用管理中心] -> [10 修改 443端口复用参数]` |
| Caddy/证书维护 | `主菜单 [19 443端口复用管理中心] -> [12 CF DNS / Caddy 证书维护]` |
| 回滚 443 配置 | `主菜单 [19 443端口复用管理中心] -> [12 CF DNS / Caddy 证书维护] -> [6 回滚 443端口复用配置]` |
...
如果之前手动改过 `/etc/caddy/conf.d`、`/etc/nginx/conf.d/vps_sni_web_*.conf` 或旧 `[4 反代]` 生成的 `/etc/nginx/conf.d/vps_proxy_*.conf`，先通过 `[8 管理 Web 域名/反代]` 或 `[10 修改 443端口复用参数]` 把参数同步到脚本，再切换。切换到 Nginx 本地 Web 反代会隔离脚本管理的 Caddy 443 Web 配置；切换回 Caddy 会隔离脚本管理的 Nginx 本地 Web 配置。重新应用 443 时也会隔离旧 Nginx HTTPS 公网反代配置，避免继续抢公网 `443`。
...
```text
主菜单 [19 443端口复用管理中心] -> [12 CF DNS / Caddy 证书维护] -> [15 隔离旧 Caddy 配置]
...
| 问题 | 菜单路径 |
|---|---|
| 只是 Caddy 语法或重载问题 | `主菜单 [19 443端口复用管理中心] -> [12 CF DNS / Caddy 证书维护] -> [12 校验并重载 Caddy]` |
| Nginx 本地 Web 反代语法或重载问题 | `主菜单 [19 443端口复用管理中心] -> [6 重新应用当前入口模式]` |
| 需要切换 Caddy/Nginx Web 反代引擎 | `主菜单 [19 443端口复用管理中心] -> [8 管理 Web 域名/反代] -> [8 切换 Web 反代引擎]` |
| 后端端口或路径写错 | `主菜单 [19 443端口复用管理中心] -> [10 修改 443端口复用参数] -> [1 修改面板/订阅端口与路径]` |
| 证书文件或软链接异常 | `主菜单 [19 443端口复用管理中心] -> [12 CF DNS / Caddy 证书维护] -> [10 重建 /root/cert 证书软链接]` |
| 不知道是哪类问题 | `主菜单 [19 443端口复用管理中心] -> [13 443 链路体检]` |
...
主菜单 [19 443端口复用管理中心] -> [12 CF DNS / Caddy 证书维护] -> [9 重新签发某个域名证书]
主菜单 [19 443端口复用管理中心] -> [12 CF DNS / Caddy 证书维护] -> [13 Caddy/证书一键体检]
```
...
主菜单 [19 443端口复用管理中心] -> [10 修改 443端口复用参数] -> [1 修改面板/订阅端口与路径]
主菜单 [19 443端口复用管理中心] -> [13 443 链路体检]
```
...
```text
主菜单 [19 443端口复用管理中心] -> [11 订阅链接 / External Proxy 提示]
主菜单 [19 443端口复用管理中心] -> [10 修改 443端口复用参数] -> [1 修改面板/订阅端口与路径]
```
...
```text
主菜单 [19 443端口复用管理中心] -> [12 CF DNS / Caddy 证书维护] -> [6 回滚 443端口复用配置]
```
