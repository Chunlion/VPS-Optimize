...
```text
主菜单 [15 配置备份与回滚] -> [2 查看现有备份列表]
主菜单 [15 配置备份与回滚] -> [3 从备份一键回滚]
```
...
| 目标 | 菜单路径 |
|---|---|
| 新增或删除网站 | `主菜单 [18 443 单入口管理中心] -> [2 管理网站/反代域名]` |
| 重新生成配置 | `主菜单 [18 443 单入口管理中心] -> [4 重新应用上次配置]` |
| 修改面板/订阅/REALITY 参数 | `主菜单 [18 443 单入口管理中心] -> [7 修改 443 分流参数]` |
| Caddy 和证书修复 | `主菜单 [18 443 单入口管理中心] -> [6 CF DNS / Caddy 证书维护]` |
| 回滚 443 配置 | `主菜单 [18 443 单入口管理中心] -> [6 CF DNS / Caddy 证书维护] -> [6 回滚 443 单入口配置]` |
...
```text
主菜单 [18 443 单入口管理中心] -> [6 CF DNS / Caddy 证书维护] -> [15 隔离旧 Caddy 配置]
...
| 问题 | 菜单路径 |
|---|---|
| 只是 Caddy 语法或重载问题 | `主菜单 [18 443 单入口管理中心] -> [6 CF DNS / Caddy 证书维护] -> [12 校验并重载 Caddy]` |
| 后端端口或路径写错 | `主菜单 [18 443 单入口管理中心] -> [7 修改 443 分流参数] -> [1 修改面板/订阅端口与路径]` |
| 证书文件或软链接异常 | `主菜单 [18 443 单入口管理中心] -> [6 CF DNS / Caddy 证书维护] -> [10 重建 /root/cert 证书软链接]` |
| 不知道是哪类问题 | `主菜单 [18 443 单入口管理中心] -> [6 CF DNS / Caddy 证书维护] -> [13 Caddy/证书一键体检]` |
...
主菜单 [18 443 单入口管理中心] -> [6 CF DNS / Caddy 证书维护] -> [9 重新签发某个域名证书]
主菜单 [18 443 单入口管理中心] -> [6 CF DNS / Caddy 证书维护] -> [13 Caddy/证书一键体检]
```
...
主菜单 [18 443 单入口管理中心] -> [7 修改 443 分流参数] -> [1 修改面板/订阅端口与路径]
主菜单 [18 443 单入口管理中心] -> [3 443 单入口链路体检]
```
...
```text
主菜单 [18 443 单入口管理中心] -> [5 订阅链接 / External Proxy 提示]
主菜单 [18 443 单入口管理中心] -> [7 修改 443 分流参数] -> [1 修改面板/订阅端口与路径]
```
...
```text
主菜单 [18 443 单入口管理中心] -> [7 修改 443 分流参数] -> [2 修改 REALITY 本地监听 / 伪装 SNI]
主菜单 [18 443 单入口管理中心] -> [4 重新应用上次配置]
主菜单 [18 443 单入口管理中心] -> [3 443 单入口链路体检]
```
...
```text
主菜单 [18 443 单入口管理中心] -> [6 CF DNS / Caddy 证书维护] -> [6 回滚 443 单入口配置]
```
