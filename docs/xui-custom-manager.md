...
不要在非 3x-ui v2.9.4 上强行写库。不要把未验证版本当作兼容版本使用。如果脚本提示当前版本未适配，停止写库操作，先保留备份和诊断信息。

## 运行前必须做的事

1. 创建 VPS 快照。
2. 备份数据库 `/etc/x-ui/x-ui.db`。
3. 确认当前 3x-ui 版本是 v2.9.4。
...
不适配版本禁止使用真实写库校准。当前仅适配并验证 3x-ui v2.9.4。
...
停止写库操作。当前仅适配并验证 3x-ui v2.9.4，不要为了其它版本猜测字段结构。可以执行备份、查看、预览、自检，并保留错误输出用于排查。
...
确认当前 3x-ui 是 v2.9.4，且 schema 检查通过后，再从菜单中启用自动检查。
