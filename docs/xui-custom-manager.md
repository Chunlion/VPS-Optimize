# 3x-ui 外置增强管理 xui-custom-manager.sh 使用说明


## 功能定位

`xui-custom-manager.sh` 是 3x-ui 外置增强工具，不替换 3x-ui 程序，也不是 3x-ui 面板本身。它用于补充一些更适合脚本处理的维护功能：
...
当前适配并验证 3x-ui v2.9.4 和 v3.3.1。

其它 3x-ui 版本不保证兼容。不适配版本不应执行写库功能，只建议执行：
...
不要在不适配的 3x-ui 版本上强行写库。不要把未验证版本当作兼容版本使用。如果脚本提示当前版本未适配，停止写库操作，先保留备份和诊断信息。

## 运行前必须做的事

1. 创建 VPS 快照。
2. 备份数据库 `/etc/x-ui/x-ui.db`。
3. 确认当前 3x-ui 版本是 v2.9.4 或 v3.3.1。
...
3x-ui v3.3.1 的客户端流量按 `email` 共享；同一客户端挂到多个入站时，客户端流量重置会作用于该 `email` 的共享记录。
...
不适配版本禁止使用真实写库校准。当前适配并验证 3x-ui v2.9.4 和 v3.3.1。
...
timer 可以在菜单中启用或停用。不适配版本不应自动安装或启用 timer。
...
停止写库操作。当前适配并验证 3x-ui v2.9.4 和 v3.3.1，不要为了其它版本猜测字段结构。可以执行备份、查看、预览、自检，并保留错误输出用于排查。
...
确认当前 3x-ui 是 v2.9.4 或 v3.3.1，且 schema 检查通过后，再从菜单中启用自动检查。
