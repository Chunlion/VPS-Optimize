...
::: warning 分享诊断前脱敏
公开前删除 Token、密码、私钥和订阅密钥；完整订阅链接也可能包含密钥。
:::
...
| 现象 | 检查方向 |
|---|---|
| [重定向循环](#err-too-many-redirects) | HTTPS 与重定向设置 |
| [空响应](#err-empty-response) | 访问地址与 SNI 路由 |
| [连接关闭或 TLS 错误](#err-connection-closed-err-ssl-protocol-error) | 公网监听与转发目标 |
| [页面不存在](#_404) | 面板或订阅路径 |
| [网关错误](#_502) | 本地后端与反代配置 |
...
下面的 `8443`、`1443`、`40000`、`2096` 是示例端口，以实际配置为准。只检查当前使用的入口和 Web 引擎；未安装或未启用的 Nginx/Caddy 不应作为当前链路故障的依据。
...
- 面板与反代的 HTTP/HTTPS 或路径重定向规则相互冲突。
- 反代向后端发送 HTTP，后端又将请求重定向到同一个公网 HTTPS 地址。
- Cloudflare 使用 Flexible 回源，而源站又将 HTTP 重定向到 HTTPS。
...
- 采用本文的本地 HTTP 后端方案时，先准备好反代和恢复入口，再清空 3x-ui 面板证书路径并重启面板。
- 让 Web 反代引擎反代到本地 HTTP 后端。
- 使用 Cloudflare 代理的 Web 域名：确认源站证书有效，SSL/TLS 模式设为 Full (strict)。临时改为 DNS only 可用于定位链路；开启代理本身不代表配置错误。参见 [Cloudflare 重定向排错](https://developers.cloudflare.com/ssl/troubleshooting/too-many-redirects/)。
...
主菜单 [19 443端口复用管理中心] -> [8 修改 443端口复用参数]
主菜单 [19 443端口复用管理中心] -> [11 443 链路体检]
```
...
```text
主菜单 [19 443端口复用管理中心] -> [8 修改 443端口复用参数] -> [5 重新应用当前保存的配置]
主菜单 [19 443端口复用管理中心] -> [11 443 链路体检]
```
...
- 后端监听端口和脚本配置不一致。
- 自定义网站/反代的 Docker 或内网后端地址无法从 Caddy/Nginx 所在主机访问。
- 后端只监听公网地址或只监听 IPv6。
...
- 启动或重启 3x-ui / x-ui。
- 面板/订阅后端：在 `主菜单 [19 443端口复用管理中心] -> [8 修改 443端口复用参数]` 修正监听地址和端口。
- 自定义网站/反代后端：在 `主菜单 [19 443端口复用管理中心] -> [6 管理 Web 域名/反代] -> [3 修改网站/反代后端]` 修正后端地址和端口。
- 重新应用配置并体检。

如果按实际后端地址测试能通，但公网仍是 502，通常是 Caddy/Nginx 反代地址或端口和实际监听不一致。
...
```text
主菜单 [1 运维预检与风险扫描]
主菜单 [19 443端口复用管理中心] -> [10 CF DNS / Caddy 证书维护]
主菜单 [19 443端口复用管理中心] -> [11 443 链路体检]
```
...
- Web 反代引擎本地监听、3x-ui 面板/订阅和 Xray/TCP/SNI 入站继续使用 `127.0.0.1` 这类本地地址。
- 自定义网站/反代后端默认使用 `127.0.0.1`；需要连接内网服务时，可填写当前 VPS 能直接访问的内网 IP 或 hostname。
- 重新应用 443 配置。
