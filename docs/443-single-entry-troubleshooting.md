...
主菜单 [19 443端口复用管理中心] -> [10 修改 443端口复用参数]
主菜单 [19 443端口复用管理中心] -> [13 443 链路体检]
```
...
```text
主菜单 [19 443端口复用管理中心] -> [10 修改 443端口复用参数] -> [5 重新应用当前保存的配置]
主菜单 [19 443端口复用管理中心] -> [13 443 链路体检]
```
...
- 启动或重启 3x-ui / x-ui。
- 面板/订阅后端：在 `主菜单 [19 443端口复用管理中心] -> [10 修改 443端口复用参数]` 修正监听地址和端口。
- 自定义网站/反代后端：在 `主菜单 [19 443端口复用管理中心] -> [8 管理 Web 域名/反代] -> [3 修改网站/反代后端]` 修正后端地址和端口。
- 重新应用配置并体检。
