...
| 你的目标 | 推荐路径 |
|---|---|
| 新买 VPS，先安全初始化 | `主菜单 [n 新手向导] -> [1 新机器初始化]` |
...
第一次使用时，按“先检查、再部署、最后验证”的顺序走，后面排错会轻松很多。

1. 新机器先跑 `主菜单 [1 运维预检与风险扫描]`，再做 `主菜单 [2 基础环境初始化]`。
2. 面板、节点和订阅工具集中在 `主菜单 [5 面板、节点与订阅工具]`。
3. 443 相关配置统一进 `主菜单 [19 443 单入口管理中心]`。
4. 配完后跑 `主菜单 [15 服务健康总览]`，确认服务、端口和证书状态。
5. 重要操作前后都建议进 `主菜单 [16 配置备份与回滚]` 做备份。
...
流量达量关机保护不单独成文档，入口在 `主菜单 [10 网络与内核优化] -> [7 流量达量关机保护]`。
