...
| 只想部署 3x-ui + REALITY | `主菜单 [1 运维预检与风险扫描] -> [4 面板、节点与订阅工具] -> [18 443 单入口管理中心]` |
| 已有面板，想接入 443 单入口 | 先看 [`docs/443-single-entry.md`](docs/443-single-entry.md)，再跑 `主菜单 [18 443 单入口管理中心] -> [1 首次配置 443 单入口]` |
| 面板打不开 / 订阅 404 / 证书失败 | `主菜单 [18 443 单入口管理中心] -> [3 443 单入口链路体检]`，再看 [`docs/443-single-entry-troubleshooting.md`](docs/443-single-entry-troubleshooting.md) |
| 只想看流量统计 | 单独运行 `dog.sh` 或进入 `主菜单 [4 面板、节点与订阅工具] -> [14 端口流量监控]` |
...
| 目标 | 推荐顺序 |
| --- | --- |
| 新机器基础初始化 | `主菜单 [1 运维预检与风险扫描]` → `[2 基础环境初始化]` → `[5 SSH 安全中心]` → `[6 Fail2ban 防爆破]` → `[7 防火墙规则管理]` → `[9 网络与内核优化]` → `[14 服务健康总览]` → `[15 配置备份与回滚]` |
| 部署面板、节点和订阅工具 | `主菜单 [1 运维预检与风险扫描]` → `[3 基础组件与反代分流]` → `[4 面板、节点与订阅工具]` → `[18 443 单入口管理中心]` → `[14 服务健康总览]` → `[15 配置备份与回滚]` |
| 后续维护 | `主菜单 [14 服务健康总览]` → `[12 端口排查与释放]` → `[9 网络与内核优化] -> [7 流量达量关机保护]` → `[15 配置备份与回滚]` → `[16 更新脚本]` → `[18 443 单入口管理中心]` |
| 新增网站或反代域名 | `主菜单 [18 443 单入口管理中心] -> [2 管理网站/反代域名]` |
...
主菜单按使用频率分组。新机器先从 `1` 开始，面板和节点进 `4`，所有 443 相关操作进 `18`。

| 你想做什么 | 入口 | 说明 |
| --- | --- | --- |
| 新机器先体检 | `主菜单 [1 运维预检与风险扫描]` | 看系统、端口、DNS、磁盘、内存和关键命令 |
| 做基础初始化 | `主菜单 [2 基础环境初始化]` | 安装常用工具、设置时区、系统更新、基础 BBR |
| 安装 Docker/Caddy/WARP | `主菜单 [3 基础组件与反代分流]` | 基础组件、普通 Caddy 反代和 443 向导入口 |
| 安装面板、节点、订阅工具 | `主菜单 [4 面板、节点与订阅工具]` | 3x-ui、S-UI、Sing-box、Xray、订阅管理、Dockge、Komari |
| 改 SSH 端口 | `主菜单 [5 SSH 安全中心] -> [1 修改 SSH 端口]` | 改前先放行云厂商安全组新端口；会同步 `sshd_config.d` 与已启用的 `ssh.socket/sshd.socket` |
| 管理 SSH 密钥登录 | `主菜单 [5 SSH 安全中心] -> [2 用户密钥登录模式]` | 添加用户公钥、切换密钥优先或仅密钥登录；会同步 `sshd_config.d`，并处理 `50-cloud-init.conf` 等云镜像限制 |
| 管理防火墙端口 | `主菜单 [7 防火墙规则管理]` | 放行、删除、查看、关闭系统防火墙规则 |
| 修改主机名 | `主菜单 [8 系统开关与清理] -> [6 修改主机名]` | 修改运行时主机名，并同步 `/etc/hostname` 和 `/etc/hosts` |
| 修改本机 hosts | `主菜单 [8 系统开关与清理] -> [7 本机 hosts 解析管理]` | 修改当前 VPS 的 `/etc/hosts` 本机解析，自动备份 |
| DNS 更改优化 | `主菜单 [9 网络与内核优化] -> [6 DNS 更改优化]` | 国内/国外默认 DNS，也可自定义 IPv4 和 IPv6 DNS |
| 网络与内核优化 | `主菜单 [9 网络与内核优化]` | BBR/TCP/ZRAM/轻量内核 |
| 防止流量超额账单 | `主菜单 [9 网络与内核优化] -> [7 流量达量关机保护]` | 自动推荐活跃网卡，按账单周期和阈值自动关机，也支持只写日志测试 |
| 管理网卡 | `主菜单 [9 网络与内核优化] -> [8 网卡管理工具]` | 查看网卡、路由、DNS、MTU，必要时刷新 DHCP |
| 测速和质量检测 | `主菜单 [11 测速与质量检测]` | YABS、流媒体、回程、IP 质量，以及 443 面板/订阅链路测试 |
| 排查端口占用 | `主菜单 [12 端口排查与释放]` | 查看监听端口，必要时释放端口 |
| 查看服务是否正常 | `主菜单 [14 服务健康总览]` | 服务状态、证书摘要、监听端口概览、443/Caddy/3x-ui/订阅工具场景概览 |
| 备份或回滚配置 | `主菜单 [15 配置备份与回滚]` | 重要操作前建议先备份 |
| 更新脚本 | `主菜单 [16 更新脚本]` | 从 GitHub 拉取最新发布版；主菜单会缓存式自动检查更新 |
| 部署 3x-ui + REALITY + 443 | `主菜单 [18 443 单入口管理中心] -> [1 首次配置 443 单入口]` | 首次配置 443 单入口 |
| 后续新增网站或反代域名 | `主菜单 [18 443 单入口管理中心] -> [2 管理网站/反代域名]` | 不需要重跑首次配置 |
| 限制面板域名访问 IP | `主菜单 [18 443 单入口管理中心] -> [8 管理域名 IP 白名单]` | 只限制被选择的域名，不影响其他 SNI |
| 普通 Caddy 域名访问 IP 限制 | `主菜单 [3 基础组件与反代分流] -> [21 普通 Caddy 域名 IP 白名单]` | 未启用 443 单入口时使用 |
| 443/证书/面板打不开 | `主菜单 [18 443 单入口管理中心] -> [3 443 单入口链路体检]` 或 `[6 CF DNS / Caddy 证书维护]` | 先体检链路，再进证书维护 |
| 检查容器是否绕过 443 | `主菜单 [10 Docker 安全管理] -> [4 Docker 端口暴露审计]` | 查看订阅工具/Dockge/Komari 是否直接暴露公网端口 |
...
面板域名或某个站点域名需要只允许固定 IP 访问时，用 `主菜单 [18 443 单入口管理中心] -> [8 管理域名 IP 白名单]`。白名单按域名单独保存，只会影响被选择的 SNI，不会收紧其他站点、REALITY 或默认分流。设置时脚本会自动尝试把 VPS 本机公网 IP 一并加入。

需要验证访问是否打通时，用 `主菜单 [18 443 单入口管理中心] -> [10 443 网络访问测试]`。它会检查 DNS、TCP、TLS SNI、面板路径、普通订阅路径和 Clash/Mihomo 路径，适合排查“面板打不开、订阅 404、REALITY 端口混乱”等问题。
...
SublinkPro、妙妙屋、Sub-Store、Dockge、Komari 都按“本地监听 + Caddy/443 对外”的思路管理。新部署的订阅工具默认优先绑定 `127.0.0.1`，公网 HTTPS 访问建议通过 `主菜单 [18 443 单入口管理中心] -> [2 管理网站/反代域名]` 添加域名；如果担心旧容器绕过 443 暴露端口，可进入 `主菜单 [10 Docker 安全管理] -> [4 Docker 端口暴露审计]` 检查。
...
Komari 默认部署到 `/opt/komari`，数据保存在 `/opt/komari/data`。安装时可选择自定义初始管理员账号和密码；如果不自定义，安装完成后可查看容器日志获取默认管理员账号。如需 HTTPS 访问，未启用 443 单入口时可用 `主菜单 [3 基础组件与反代分流] -> [13 普通 Caddy 反代]`；已启用 443 单入口时再用 `主菜单 [18 443 单入口管理中心] -> [2 管理网站/反代域名]` 添加反代域名。
...
如果 VPS 套餐有月流量限制，建议进入 `主菜单 [9 网络与内核优化] -> [7 流量达量关机保护]` 配置保护。它会安装本地 `systemd timer`，按网卡统计本周期流量，达到阈值后自动关机，避免被刷流量后产生超额账单。
...
高风险功能会要求输入 `YES`。不确定时先做 `主菜单 [15 配置备份与回滚]`。
