...
| 已有面板，想接入 443 单入口 | 先看 [`docs/443-single-entry.md`](docs/443-single-entry.md)，再跑 `主菜单 [19 443 单入口管理中心] -> [2 首次配置 / 安装 443 单入口]` |
| 面板打不开 / 订阅 404 / 证书失败 | `主菜单 [19 443 单入口管理中心] -> [11 443 链路体检]`，再看 [`docs/443-single-entry-troubleshooting.md`](docs/443-single-entry-troubleshooting.md) |
...
| 安装面板、节点、订阅工具 | `主菜单 [5 面板、节点与订阅工具]` | 3x-ui、Sing-box、Xray、订阅管理、Dockge、Komari |
...
| 部署 3x-ui + REALITY + 443 | `主菜单 [19 443 单入口管理中心] -> [2 首次配置 / 安装 443 单入口]` | 首次配置 443 单入口 |
| 后续新增网站或反代域名 | `主菜单 [19 443 单入口管理中心] -> [8 管理 Web 域名/反代]` | 不需要重跑首次配置 |
| 限制面板域名访问 IP | `主菜单 [19 443 单入口管理中心] -> [9 管理 Web 域名 IP 白名单]` | 只限制被选择的域名，不影响其他 SNI |
| 普通 Caddy 域名访问 IP 限制 | `主菜单 [4 普通 Caddy 反代] -> [4 普通 Caddy 域名 IP 白名单]` | 未启用 443 单入口时使用 |
| 443/证书/面板打不开 | `主菜单 [19 443 单入口管理中心] -> [11 443 链路体检]` 或 `[13 CF DNS / Caddy 证书维护]` | 先体检链路，再进证书维护 |
...
TCP Peek + Splice 模式：配置过程和 Nginx Stream 一样，复用同一套域名、证书、Caddy 后端、Web 白名单和 Xray SNI 分流记录。它基于 MSG_PEEK 读取 TLS ClientHello 中的 SNI，不消费首包，后端仍收到原始 TLS 握手；转发时优先使用 splice 零拷贝，失败时自动回退普通 copy。实际运行的分流器程序为 vpso-mux。
...
TCP Peek + Splice 切换前必须先跑 `主菜单 [19] -> [16] 查看 TCP Peek + Splice 状态 / 8444 预检` 和 `[17] TCP Peek 分流规则校验`。正式切换使用 `[5] 切换到 TCP Peek + Splice 模式`，不会在公网 `443` 切换路径里自动下载 Go 工具链或远端编译 `vpso-mux`；如果当前 SSH 正连在入口端口，例如 `443`，脚本会拒绝切换，避免把当前管理连接踢掉。需要撤销最近一次入口模式切换时，统一用 `[7] 回滚上一次入口模式切换`。
...
如果机器上已经有普通 Caddy 反代，启用 443 单入口前请先备份并记录旧站点的域名、后端地址和端口。脚本会隔离可能抢占公网 `443` 的旧 Caddy 配置，但不会自动把旧反代规则迁移成新的 443 单入口站点；启用后需要通过 `主菜单 [19 443 单入口管理中心] -> [8 管理 Web 域名/反代]` 手动补录旧网站。

面板域名或某个站点域名需要只允许固定 IP 访问时，用 `主菜单 [19 443 单入口管理中心] -> [9 管理 Web 域名 IP 白名单]`。白名单按 Web 域名单独保存，只会影响被选择的 Caddy/Web SNI，不会收紧 Xray 入站、REALITY 或默认分流。设置时脚本会自动尝试把 VPS 本机公网 IP 一并加入。
...
SublinkPro、妙妙屋、Sub-Store、Dockge、Komari 都按“本地监听 + Caddy/443 对外”的思路管理。新部署的订阅工具默认优先绑定 `127.0.0.1`，公网 HTTPS 访问建议通过 `主菜单 [19 443 单入口管理中心] -> [8 管理 Web 域名/反代]` 添加域名；如果担心旧容器绕过 443 暴露端口，可进入 `主菜单 [11 Docker 安全管理] -> [4 Docker 端口暴露审计]` 检查。

面板/订阅工具账号提示：

- 3x-ui / x-ui 使用官方安装器，管理员账号、密码和面板路径由官方安装器交互设置或在安装结束时输出，请留意并保存。
...
Komari 默认部署到 `/opt/komari`，数据保存在 `/opt/komari/data`。安装时可选择自定义初始管理员账号和密码；如果不自定义，安装完成后可查看容器日志获取默认管理员账号。如需 HTTPS 访问，未启用 443 单入口时可用 `主菜单 [4 普通 Caddy 反代] -> [1 添加普通 Caddy 反代]`；已启用 443 单入口时再用 `主菜单 [19 443 单入口管理中心] -> [8 管理 Web 域名/反代]` 添加反代域名。
