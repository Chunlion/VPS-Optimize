...
Komari 默认部署到 `/opt/komari`，数据保存在 `/opt/komari/data`。安装时可选择自定义初始管理员账号和密码；如果不自定义，安装完成后可查看容器日志获取默认管理员账号。如需 HTTPS 访问，建议通过 `主菜单 [19 443 单入口管理中心] -> [2 管理网站/反代域名]` 添加 443 单入口反代域名。
