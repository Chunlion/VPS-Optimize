...
| 已经启用 443 单入口 | `主菜单 [19 443 单入口管理中心] -> [2 管理网站/反代域名]` 新增反代域名 |
...
| 443 单入口新增域名失败 | 使用脚本自动备份回滚，或从 `主菜单 [19 443 单入口管理中心] -> [2 管理网站/反代域名]` 删除该域名 |
| 证书失败 | `主菜单 [19 443 单入口管理中心] -> [6 CF DNS / Caddy 证书维护]` 检查 Token、DNS、重签 |
