...
| 已经启用 443端口复用 | `主菜单 [19 443端口复用管理中心] -> [8 管理 Web 域名/反代]` 新增反代域名；需要时用同菜单 `[8 切换 Web 反代引擎]` 在 Caddy/Nginx 间切换 |
...
如果你之前在 `主菜单 [4 反代]` 配过独立 Caddy/Nginx HTTPS 反代，启用或重新应用 443端口复用时，脚本会把可能抢占公网 `443` 的旧配置隔离。之后订阅工具域名都应从 `[19] -> [8]` 补录，不要再让 `[4 反代]` 的 Nginx HTTPS 反代直接监听公网 `443`。
...
| 443端口复用新增域名失败 | 使用脚本自动备份回滚，或从 `主菜单 [19 443端口复用管理中心] -> [8 管理 Web 域名/反代]` 删除该域名 |
| 证书失败 | `主菜单 [19 443端口复用管理中心] -> [12 CF DNS / Caddy 证书维护]` 检查 Token、DNS、重签 |
