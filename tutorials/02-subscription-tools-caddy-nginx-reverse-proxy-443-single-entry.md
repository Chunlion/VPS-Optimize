...
| 当前状态 | 推荐方式 |
|---|---|
| 还没有启用 443端口复用，只想先访问订阅工具 | `主菜单 [4 反代]`，按现有环境选择 Caddy 或 Nginx HTTPS 反代 |
| 已经启用 443端口复用 | `主菜单 [19 443端口复用管理中心] -> [6 管理 Web 域名/反代]` 新增反代域名；需要时用同菜单 `[8 切换 Web 反代引擎]` 在 Caddy/Nginx 间切换 |
| 订阅工具只给自己用 | 后端只监听本机或内网地址，外部通过 Caddy/Nginx 或 443端口复用访问 |
...
| 后端地址/端口 | `127.0.0.1:3000`、`10.0.0.20:3000` 等 | 本机或内网后端的示例值，请换成实际值 |
...
| 预检 | 2-5 分钟 |
| 部署订阅工具 | 5-20 分钟 |
| 配置 Caddy/Nginx 或 443端口复用 | 5-15 分钟 |
| 验证订阅输出 | 5-10 分钟 |
| 备份 | 1-3 分钟 |
...
| Nginx HTTPS 反代 | 未启用 443端口复用时可新增 HTTPS 站点配置 | 配置错误或端口冲突会导致 80/443 访问异常 |
| Nginx stream | 如果接入 443端口复用，会新增 SNI 分流 | 配置错误可能影响公网 443 |
...
订阅工具后端建议只监听本机或内网，不要直接暴露到公网。以下均为示例值：
...
Docker 防穿透会修改 Docker 网络行为并重启 Docker。确认容器不依赖公网直连端口后再继续。

### 4A. 方案一：未启用 443端口复用时使用 Caddy/Nginx 反代

适合还没启用 443端口复用，只想先用域名访问订阅工具。
...
| `[2 添加 Nginx HTTPS 反代]` | 未启用 443端口复用，且希望由 Nginx 直接监听公网 80/443 提供 HTTPS |
...
`sub.example.com` 是示例值，请替换成实际订阅域名。Nginx HTTPS 反代只适用于未启用 443端口复用的场景；如果脚本检测到 443端口复用配置，会拒绝继续，避免 Nginx 抢占公网 `443`。同一域名不要同时交给 Caddy 和 Nginx 接管。
...
Docker 服务建议把容器端口发布到宿主机 `127.0.0.1`，再填写对应的宿主机端口。连接其它内网服务器时，可以填写实际内网 IP 或 hostname，但必须先确认当前 VPS 能直接访问。

脚本会按当前 Web 反代引擎更新 Caddy 或 Nginx 本地配置并申请证书。出现高风险确认时，先确认快照、DNS、Token 和后端端口无误，再输入大写 `YES`。

如果你之前在 `主菜单 [4 反代]` 配过独立 Caddy/Nginx HTTPS 反代，启用或重新应用 443端口复用时，脚本会把可能抢占公网 `443` 的旧配置隔离。之后订阅工具域名都应从 `[19] -> [6]` 补录，不要再让 `[4 反代]` 的 Nginx HTTPS 反代直接监听公网 `443`。
...
未启用 443端口复用的 Caddy/Nginx 反代，按实际使用的入口验证。
...
| 443端口复用新增域名失败 | 使用脚本自动备份回滚，或从 `主菜单 [19 443端口复用管理中心] -> [6 管理 Web 域名/反代]` 删除该域名 |
| 证书失败 | `主菜单 [19 443端口复用管理中心] -> [10 CF DNS / Caddy 证书维护]` 检查 Token、DNS、重签 |
