...
| --- | --- |
| 打开管理菜单 | `xcm` |
| 预览自定义重置 | 菜单里的“预览并手动执行一次重置检查”，或 `--reset-check --dry-run` |
| 真实执行一次检查 | `--reset-check` |
| 查看是否有 monthly 冲突 | 健康检查 / 预览模式 |
| 校准已用流量 | 校准已用流量菜单 |
| 恢复旧状态 | 备份恢复菜单 |
...
如果外置管理的入站仍启用 monthly，菜单、预览模式和健康检查会显示提醒
```
...
菜单里的“自定义流量重置日期 -> 预览并手动执行一次重置检查”会先进入预览模式。
