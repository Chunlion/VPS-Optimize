...
func_env_install() {
    while true; do
        ...
        echo -e "------------------------------------------------"
        echo -e "${CYAN} 13. 一键配置 Caddy 反向代理 ${YELLOW}(域名+自动HTTPS)${PLAIN}"
        echo -e "------------------------------------------------"
        echo -e "${RED}  0. 返回主菜单${PLAIN}"
        echo -e "${CYAN}================================================${PLAIN}"
        ...
        case $env_choice in
            ...
            7) curl -L https://raw.githubusercontent.com/naiba/nezha/master/script/install.sh -o nezha.sh && chmod +x nezha.sh && ./nezha.sh; echo -e "\n${YELLOW}💡 面板自定义代码去动画提示：\n<script>\nwindow.ShowNetTransfer = true;\nwindow.FixedTopServerName = true;\nwindow.DisableAnimatedMan = true;\n</script>${PLAIN}" ;;
            ...
            13) if [[ "$OS" =~ debian|ubuntu ]]; then apt install -y debian-keyring debian-archive-keyring apt-transport-https -qq && curl -1sLf 'https://dl.cloudsmith.io/public/caddy/stable/gpg.key' | gpg --dearmor -o /usr/share/keyrings/caddy-stable-archive-keyring.gpg && curl -1sLf 'https://dl.cloudsmith.io/public/caddy/stable/debian.deb.txt' | tee /etc/apt/sources.list.d/caddy-stable.list && apt update && apt install caddy -y; else yum install -y yum-utils && yum-config-manager --add-repo https://openrepo.io/repo/caddy/caddy.repo && yum install caddy -y; fi; read -p "请输入解析后的域名: " domain; read -p "请输入本地端口 (如 2053): " port; echo -e "$domain {\n    reverse_proxy localhost:$port\n}" > /etc/caddy/Caddyfile; systemctl restart caddy && echo -e "${GREEN}✅ 反代成功！访问 https://$domain${PLAIN}" ;;
            0) break ;;
        esac
...
# ---------------------------------------------------------
# 4. SSH 安全 / 5. Docker 深度 / 6. BBR 管理
# ---------------------------------------------------------
func_security() {
    clear
    current_p=$(grep -i "^Port" /etc/ssh/sshd_config | awk '{print $2}' | head -n1)
    current_p=${current_p:-22}
    read -p "当前端口 $current_p, 请输入新端口 (回车保持): " final_p; final_p=${final_p:-$current_p}
    if [[ "$final_p" != "$current_p" ]]; then
        sed -i "s/^Port .*/Port $final_p/" /etc/ssh/sshd_config; grep -q "^Port $final_p" /etc/ssh/sshd_config || echo "Port $final_p" >> /etc/ssh/sshd_config
        ufw allow "$final_p"/tcp >/dev/null 2>&1; firewall-cmd --permanent --add-port="$final_p"/tcp >/dev/null 2>&1; firewall-cmd --reload >/dev/null 2>&1
...
func_bbr_manage() {
    clear; wget -O tcpx.sh "https://github.com/ylx2016/Linux-NetSpeed/raw/master/tcpx.sh" && chmod +x tcpx.sh && ./tcpx.sh
}

# ---------------------------------------------------------
# 7. TCP (Omnitt) / 8. 内存 / 9. 内核 / 10. 清理旧内核
# ---------------------------------------------------------
func_tcp_tune() {
    clear; echo -e "请浏览器打开: ${BLUE}https://omnitt.com/${PLAIN} 生成参数"
    read -p "👉 准备好粘贴了吗？(y/n): " yn; if [[ ! "$yn" =~ ^[Yy]$ ]]; then return; fi
    temp_f="/etc/sysctl.d/99-omnitt-tune.conf"; > "$temp_f"
    echo -e "\n${CYAN}👇 请右键粘贴代码，完成后在新行输入 EOF 并回车：${PLAIN}"
    while IFS= read -r line; do line=$(echo "$line" | tr -d '\r'); [[ "$line" == "EOF" ]] && break; echo "$line" >> "$temp_f"; done
    if [ -s "$temp_f" ]; then sysctl -p "$temp_f" >/dev/null; echo -e "${GREEN}✅ 应用成功！${PLAIN}"; fi; read -n 1 -s -r -p "按任意键继续..."
}

func_zram_swap() {
    clear; mem=$(free -m | awk '/^Mem:/{print $2}')
    ...
    echo -e "${GREEN}✅ 调优已完成！${PLAIN}"; read -n 1 -s -r -p "按任意键继续..."
}

func_install_kernel() { clear; if [[ ! "$OS" =~ debian|ubuntu ]]; then echo -e "${RED}仅支持 Debian/Ubuntu${PLAIN}"; else apt update -y && apt install -y linux-image-cloud-amd64; echo -e "${GREEN}✅ Cloud 内核安装完成！请重启${PLAIN}"; fi; read -n 1 -s -r -p "按任意键返回..."; }
func_clean_kernel() { clear; if [[ ! "$OS" =~ debian|ubuntu ]]; then echo -e "${RED}仅支持 Debian/Ubuntu${PLAIN}"; else echo -e "当前内核: $(uname -r)\n${RED}警告：勿删当前运行的及 cloud 内核！${PLAIN}"; dpkg --list | grep linux-image; read -p "输入旧内核包名 (回车取消): " old_k; if [ -n "$old_k" ]; then apt purge -y $old_k && update-grub && apt autoremove --purge -y; echo -e "${GREEN}✅ 清理完成！${PLAIN}"; fi; fi; read -n 1 -s -r -p "按任意键返回..."; }

# ---------------------------------------------------------
# 11. 探针 (修复版) / 12. 测试合集 / 13. 流量狗 / 14-15 面板
# ---------------------------------------------------------
func_system_info() {
    clear
...
# ---------------------------------------------------------
func_update_script() {
    clear; echo -e "${CYAN}👉 正在从 GitHub 获取最新版本...${PLAIN}"
    if curl -sL "$UPDATE_URL" -o /tmp/cy_new.sh; then
        mv /tmp/cy_new.sh "$0"; chmod +x "$0"; cp "$0" /usr/local/bin/cy
        echo -e "${GREEN}✅ 更新成功！正在重启面板...${PLAIN}"; sleep 1; exec bash "$0"
    else
        echo -e "${RED}❌ 更新失败，请检查网络！${PLAIN}"; read -n 1 -s -r -p "按任意键返回..."
    fi
}
...
main_menu() {
    ...
    while true; do
        ...
        echo -e "  ${GREEN}6.${PLAIN} BBR 加速管理     ${YELLOW}(调用 ylx2016 终极加速脚本)${PLAIN}"
        ...
        echo -e " ${GREEN}11.${PLAIN} 极速硬件探针     ${YELLOW}(查看本机配置与实时负载占用)${PLAIN}"
        ...
        case $choice in
            ...
            17) func_update_script ;; 16) reboot ;; 0) exit 0 ;;
