...
VPS-Optimize will modify system services, firewalls, kernel parameters, reverse proxy configuration, Docker configuration, certificate files and shared port 443 point related services. The backup and recovery methods must be confirmed before running the production environment.
...
4. Confirm the current listener of the public port `443` before enabling shared port 443.
...
Before modifying the SSH, firewall, shared port 443, certificate, and reverse proxy configurations, first confirm that the cloud security group allows the necessary ports.

## shared port 443 considerations

After enabling the 443 shared entry, Internet `443` should only be bound by a single service corresponding to the current entry mode at the same time:
