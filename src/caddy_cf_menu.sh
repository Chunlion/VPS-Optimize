    ...
    while true; do
        ...
        case $m_choice in
            ...
            3)
                ...
                if [[ ! -x "$acme_bin" ]]; then
                    echo -e "$(localized_text "${RED}❌ 未检测到 acme.sh，请先运行主菜单 [19] -> [2] 首次配置 443端口复用。${PLAIN}" "${RED}❌ acme.sh is not detected, please run the main menu [19] -> [2] first to configure Port 443 Reuse.${PLAIN}" "${RED}❌ acme.sh не обнаружен, запустите главное меню [19] -> [2], чтобы настроить повторное использование порта 443 в первый раз.${PLAIN}")"
                    read -n 1 -s -r -p "$(localized_text "按任意键继续..." "Press any key to continue..." "Нажмите любую клавишу, чтобы продолжить...")"
                    continue
                fi
