...
show_panel_help() {
    ...
    echo "$(localized_text "1 3x-ui 面板脚本：安装、官方菜单、修复面板。" "1 3x-ui panel script: installation, official menu, repair panel." "1 Скрипт панели 3x-ui: установка, официальное меню, панель восстановления.")"
    echo "$(localized_text "2 x-ui 增强套件：重置日期、流量校准、备份恢复和日志。" "2 x-ui Enhancement Kit: reset date, traffic correction, backup recovery and logs." "2 x-ui Enhancement Kit: сброс даты, калибровка расхода, восстановление резервной копии и журналы.")"
    ...
    echo "$(localized_text "7/8/9 订阅栈，11 Dockge Compose，12 Compose 迁移；公网 HTTPS：未启用 443 单入口走主菜单 [4 反代]，已启用走主菜单 [19 443 单入口管理中心] -> [8 管理 Web 域名/反代]。" "7/8/9 Subscription stack, 11 Dockge Compose, 12 Compose migration; Internet HTTPS: Not enabled 443 shared entry to go to the main menu [4 reverse proxy], enabled to go to the main menu [19 443 shared entry management center] -> [8 management Web domain/reverse proxy]." "Стек подписки 7/8/9, миграция 11 Dockge Compose, 12 Compose; Интернет HTTPS: Не включен общая точка входа 443 для перехода в главное меню [4 обратный прокси-сервер], включен для перехода в главное меню [19 443 центр управления общей точкой входа] -> [8 имя веб-домена управления/обратный прокси].")"
...
show_sni_help() {
    ...
    echo "$(localized_text "1 查看当前入口状态 / 监听详情：显示公网 443、Web 反代引擎、Xray 和服务状态。" "1 View entry status and listeners: public port 443, Web reverse proxy, Xray, and services." "1 Состояние входа и прослушивания: публичный порт 443, веб-прокси, Xray и службы.")"
    echo "$(localized_text "2 首次配置 / 安装：建立共享 Web 域名、Web 反代引擎、证书和默认 Nginx Stream 入口。" "2 First-time configuration/installation: Establish a shared web domain, web reverse proxy engine, certificate and default Nginx Stream entry." "2 Первоначальная настройка/установка: укажите общее имя веб-домена, механизм веб-прокси, сертификат и запись Nginx Stream по умолчанию.")"
    echo "$(localized_text "3/4/5 入口模式切换：在 Nginx Stream 模式、Xray Fallback 模式、TCP Peek + Splice 模式之间切换。" "3/4/5 Entry mode switching: Switch between Nginx Stream mode, Xray Fallback mode, and TCP Peek + Splice mode." "3/4/5 Переключение режима входа: переключение между режимом Nginx Stream, резервным режимом Xray и режимом TCP Peek + Splice.")"
    echo "$(localized_text "6 重新应用：按当前 ENTRY_MODE 重新生成并启动入口配置。" "6 Reapply: Regenerate and start the entry configuration according to the current ENTRY_MODE." "6 Повторное применение: повторно сгенерируйте и запустите конфигурацию записи в соответствии с текущим ENTRY_MODE.")"
    ...
    echo "$(localized_text "8 管理 Web 域名/反代：后续新增或删除网站，不需要重跑首次配置。" "8. Manage Web domains/reverse proxy: Subsequently adding or deleting websites does not require re-running the first configuration." "8. Управление именами веб-доменов/обратный прокси: последующее добавление или удаление веб-сайтов не требует повторного запуска первой настройки.")"
    ...
    echo "$(localized_text "10 修改 443 共享参数：调整面板、订阅、REALITY、入口端口与路径。" "10 Modify 443 shared parameters: adjustment panel, subscription, REALITY, entry port and path." "10 Измените 443 общих параметра: панель настройки, подписку, REALITY, порт входа и путь.")"
    echo "$(localized_text "11 订阅链接 / External Proxy 提示：检查节点链接是否输出公网 443。" "11 Subscription link / External Proxy Tip: Check whether the node link outputs public port 443." "11 Ссылка на подписку/внешний прокси. Совет: проверьте, выводит ли ссылка на узел публичный порт 443.")"
    echo "$(localized_text "12 CF DNS / Caddy 证书维护：重签证书、修复软链接、清理和回滚。" "12 CF DNS / Caddy Certificate maintenance: re-sign the certificate, repair symlinks, clean up and rollback." "12 CF DNS / Caddy Обслуживание сертификата: переподписка сертификата, восстановление программных ссылок, очистка и откат.")"
    echo "$(localized_text "13 链路体检：排查 ENTRY_MODE、监听、证书、Web 和 Xray 分流。" "13 Connection health check: inspect ENTRY_MODE, listeners, certificates, Web, and Xray routes." "13 Проверка соединений: ENTRY_MODE, прослушивание, сертификаты, Web и маршруты Xray.")"
    echo "$(localized_text "14 网络访问测试：检查 DNS、TCP、TLS SNI、面板和订阅路径响应。" "14 Network access test: Check DNS, TCP, TLS SNI, panel and subscription path responses." "14 Проверка доступа к сети: проверьте DNS, TCP, TLS SNI, ответы панели и пути подписки.")"
    ...
    echo "$(localized_text "16 查看 TCP Peek + Splice 状态 / 8444 预检：展示 status.json 统计；预检只监听 8444，不改公网 443。" "16 View TCP Peek + Splice status / 8444 pre-switch check: display status.json statistics; the pre-switch check only monitors 8444 and does not change the public port 443." "16 Просмотр статуса TCP Peek + Splice / 8444 предварительная проверка: отображение статуса. Статистика json; предварительная проверка только прослушивает 8444 и не меняет публичный порт 443.")"
    echo "$(localized_text "17 TCP Peek 分流规则校验：只检查配置，不重启入口。" "17 TCP Peek Routing rule verification: only checks the configuration and does not restart the entry." "17 TCP Peek Проверка правил маршрутизации: только проверяет конфигурацию и не перезапускает вход.")"
    echo "$(localized_text "18 查看 TCP Peek + Splice 日志：查看 vpso-mux 分流器日志。" "18 View the TCP Peek + Splice log: View the vpso-mux traffic router log." "18 Просмотр журнала TCP Peek + Splice: просмотр журнала маршрутизатор трафика vpso-mux.")"
...
show_backup_help() {
    ...
    echo "$(localized_text "3 回滚：会覆盖当前配置，必须输入 yes 确认，大小写均可。" "3 Rollback: The current configuration will be overwritten. You must enter yes to confirm. Both upper and lower case are acceptable." "3 Откат: Текущая конфигурация будет перезаписана. Вы должны ввести «да» для подтверждения. Допускаются как верхний, так и нижний регистр.")"
    echo "$(localized_text "4 隔离旧备份：只移动到隔离目录，不直接删除。" "4. Quarantine old backups: only move them to the quarantine directory and do not delete them directly." "4. Помещайте старые резервные копии в карантин: перемещайте их только в каталог карантина, но не удаляйте напрямую.")"
    echo "$(localized_text "5 查看/编辑脚本已应用配置：先备份，再按配置类型校验，可选择 reload/restart。" "5 View/edit the script's applied configuration: back up first, then verify according to the configuration type, and you can choose reload/restart." "5. Просмотрите/измените примененную конфигурацию сценария: сначала создайте резервную копию, затем проверьте тип конфигурации и выберите перезагрузку/перезапуск.")"
    echo "$(localized_text "? 查看帮助，0/q 返回主菜单。" "? View help, 0/q returns to the main menu." "? Просмотр справки, 0/q возвращает в главное меню.")"
}
