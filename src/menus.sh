...
show_sni_help() {
    ...
    echo "$(localized_text "2 首次配置 / 安装：配置共享 Web 域名、Web 反代引擎、证书和默认 Nginx Stream 入口。" "2 First-time setup: configure shared Web domains, the Web reverse-proxy engine, certificates, and the default Nginx Stream entry mode." "2 Первоначальная настройка: настройка общих Web-доменов, Web-обратного прокси, сертификатов и режима Nginx Stream по умолчанию.")"
    ...
    echo "$(localized_text "10 修改 443 共享参数：调整面板、订阅、REALITY、入口端口与路径。" "10 Edit shared port 443 settings: panel, subscription, REALITY, entry ports, and paths." "10 Изменить параметры общего порта 443: панель, подписку, REALITY, порты входа и пути.")"
    echo "$(localized_text "11 订阅链接 / External Proxy 提示：检查节点链接是否输出公网 443。" "11 Subscription link / External Proxy guidance: check whether node links use public port 443." "11 Справка по ссылке подписки / External Proxy: проверьте, используют ли ссылки узлов публичный порт 443.")"
    ...
    echo "$(localized_text "13 链路体检：排查 ENTRY_MODE、监听、证书、Web 和 Xray 分流。" "13 Connection diagnostics: inspect ENTRY_MODE, listeners, certificates, Web routing, and Xray routes." "13 Диагностика соединения: проверьте ENTRY_MODE, прослушивание, сертификаты, Web-маршрутизацию и маршруты Xray.")"
    ...
    echo "$(localized_text "16 查看 TCP Peek + Splice 状态 / 8444 预检：展示 status.json 统计；预检只监听 8444，不改公网 443。" "16 TCP Peek + Splice status / port 8444 preflight: show status.json statistics. The preflight listens only on 8444 and does not change public port 443." "16 Статус TCP Peek + Splice / предварительная проверка 8444: показывает статистику status.json. Проверка слушает только 8444 и не меняет публичный порт 443.")"
