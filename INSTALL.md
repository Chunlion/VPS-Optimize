...
主菜单 [9 网络与内核优化]
主菜单 [14 服务健康总览]
主菜单 [15 配置备份与回滚] -> [1 创建全量配置备份]
```

如果需要优化系统 DNS，可在网络菜单里进入：

```text
主菜单 [9 网络与内核优化] -> [6 DNS 更改优化]
```
...
| 看服务、证书、端口总览 | `主菜单 [14 服务健康总览]` |
| 修改主机名 | `主菜单 [8 系统开关与清理] -> [6 修改主机名]` |
| 排查端口占用 | `主菜单 [12 端口排查与释放]` |
| 443 链路体检 | `主菜单 [18 443 单入口管理中心] -> [3 443 单入口链路体检]` |
| Caddy/证书体检 | `主菜单 [18 443 单入口管理中心] -> [6 CF DNS / Caddy 证书维护] -> [13 Caddy/证书一键体检]` |
| 创建备份 | `主菜单 [15 配置备份与回滚] -> [1 创建全量配置备份]` |
| 查看备份 | `主菜单 [15 配置备份与回滚] -> [2 查看现有备份列表]` |
