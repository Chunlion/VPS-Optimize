...
主菜单 [5 面板、节点与订阅工具] -> [5 管理 SublinkPro]
...
如果需要优化系统 DNS，可在网络菜单里进入：

```text
主菜单 [10 网络与内核优化] -> [6 DNS 更改优化]
```

可以选择国内默认 DNS、国外默认 DNS，或自定义 IPv4/IPv6 DNS。脚本会先备份 `/etc/resolv.conf` 和自己的 `systemd-resolved` 配置，解析异常时可在同一菜单里恢复最近一次 DNS 备份。
...
```text
主菜单 [19 443 单入口管理中心] -> [8 管理网站/反代域名]
```
...
| 工具 | 菜单路径 |
|---|---|
| SublinkPro | `主菜单 [5 面板、节点与订阅工具] -> [5 管理 SublinkPro]` |
| 妙妙屋订阅管理 | `主菜单 [5 面板、节点与订阅工具] -> [6 管理 妙妙屋订阅管理]` |
| Sub-Store | `主菜单 [5 面板、节点与订阅工具] -> [7 管理 Sub-Store]` |
| Dockge | `主菜单 [5 面板、节点与订阅工具] -> [8 管理 Dockge]` |
